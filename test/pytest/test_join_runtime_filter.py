...
def test_serial_join_runtime_filter(pgc_conn,expect):
 ...
 with pgc_conn.cursor() as c:
  if _has_runtime_filter(pgc_conn):c.execute("SET pgcolumnar.enable_join_runtime_filter=on")
  c.execute(sql);a=c.fetchone()
  if _has_runtime_filter(pgc_conn):c.execute("SET pgcolumnar.enable_join_runtime_filter=off")

...
base="$(pc "SET enable_nestloop=off;SET enable_mergejoin=off;EXPLAIN(ANALYZE,TIMING off,SUMMARY off)$SQL")"
rf_set=""
if [[ "$(pc "SELECT current_setting('pgcolumnar.enable_join_runtime_filter', true) IS NOT NULL" | tail -1)" == t ]]; then
  rf_set="SET pgcolumnar.enable_join_runtime_filter=on;"
fi
on="$(pc "${rf_set}SET max_parallel_workers_per_gather=0;SET enable_nestloop=off;SET enable_mergejoin=off;EXPLAIN(ANALYZE,TIMING off,SUMMARY off)$SQL")"
...
check "filter ready before scan" "$(grep -c 'Runtime Filter Ready: yes' <<<"$on")" 1
...
check "runtime answer equals off" "$(pc "${rf_set}$SQL"|tail -1)" "$(pc "$SQL"|tail -1)"
check "runtime answer equals heap" "$(pc "${rf_set}$SQL"|tail -1)" "$(q 'SELECT count(*),sum(h.k),sum(length(h.p)) FROM h JOIN d ON h.k=d.k')"
for shape in \
 ...
 n=${shape%%|*}; s=${shape#*|}; p="$(pc "${rf_set}EXPLAIN $s")"; check "$n refusal" "$(grep -c 'Columnar Runtime Filter Coordinator'<<<"$p")" 0
done
pgc_summary
